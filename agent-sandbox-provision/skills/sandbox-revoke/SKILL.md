---
name: sandbox-revoke
description: Delete the read-only cloud sandbox provisioned by agent-sandbox-provision. Delete-only — no disable. Sub-commands delete (full teardown) and delete-account (single-account teardown). NOT YET IMPLEMENTED.
user-invocable: false
---

# Sandbox Revoke — NOT YET IMPLEMENTED

This skill is part of the read-only cloud sandbox bundle. The full implementation lands after `agent-sandbox-provision` is in place. See `~/.claude/plans/refactored-twirling-flask.md` for the design.
