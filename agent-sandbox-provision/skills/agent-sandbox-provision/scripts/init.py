#!/usr/bin/env python3
"""
init: machine-level setup for agent-sandbox-provision.
...
EDIT_MODE_HOOK_DIR = SKILL_DIR / "hooks"
EDIT_MODE_STASH_HOOK = EDIT_MODE_HOOK_DIR / "edit-mode-stash.sh"
EDIT_MODE_RESTORE_HOOK = EDIT_MODE_HOOK_DIR / "edit-mode-restore.sh"
...
def phase_local_skill_symlink(username: str, *, ctx: common.Ctx) -> None:
    ...
    skills dir, so /sandbox-cred-sweep and friends are reachable inside the
...
def phase_local_edit_mode_hooks_actions(*, ctx: common.Ctx) -> list[str]:
    return []


def phase_local_edit_mode_hooks(*, ctx: common.Ctx) -> None:
    """Install Claude Code hooks that preserve file mode across Edit calls.

    Edit creates a new inode at 0600 (mkstemp + rename), dropping g+r/o+r that
    claude-ro relies on for read access via POSIX bits, and any execute bits
    the file originally had. This phase chmods the shipped hook scripts under
    SKILL_DIR/hooks/ and registers them in ~/.claude/settings.json so a
    PreToolUse pair stashes the mode and a PostToolUse pair restores it.

    Idempotent: re-running does not duplicate settings.json entries (detected
    by script path inside the command string)."""
    if ctx.dry_run:
        common.log.info("[dry-run] would chmod 755 %s, %s and register them in %s",
                        EDIT_MODE_STASH_HOOK, EDIT_MODE_RESTORE_HOOK, CLAUDE_SETTINGS_PATH)
        return
    if not EDIT_MODE_STASH_HOOK.exists() or not EDIT_MODE_RESTORE_HOOK.exists():
        raise SystemExit(
            f"edit-mode hook scripts missing under {EDIT_MODE_HOOK_DIR}; "
            "the skill is incomplete — reinstall it."
        )
    EDIT_MODE_STASH_HOOK.chmod(0o755)
    EDIT_MODE_RESTORE_HOOK.chmod(0o755)
    _register_edit_mode_hooks_in_settings()
    common.log.info("installed edit-mode-preservation hooks in %s", CLAUDE_SETTINGS_PATH)


def _register_edit_mode_hooks_in_settings() -> None:
    """Idempotently add PreToolUse + PostToolUse Edit entries that invoke our
    mode-preservation scripts. Other hooks in the file are preserved."""
    ...
    def _already_registered(event: str, script: pathlib.Path) -> bool:
        for entry in hooks.get(event, []):
            for h in entry.get("hooks", []):
                if str(script) in h.get("command", ""):
                    return True
        return False

    if not _already_registered("PreToolUse", EDIT_MODE_STASH_HOOK):
        hooks.setdefault("PreToolUse", []).append({
            "matcher": "Edit",
            "hooks": [{
                "type": "command",
                "command": f"bash {EDIT_MODE_STASH_HOOK}",
            }],
        })
    if not _already_registered("PostToolUse", EDIT_MODE_RESTORE_HOOK):
        hooks.setdefault("PostToolUse", []).append({
            "matcher": "Edit",
            "hooks": [{
                "type": "command",
                "command": f"bash {EDIT_MODE_RESTORE_HOOK}",
            }],
        })
...
def main() -> None:
    ap = argparse.ArgumentParser(
        description="Machine-level setup for agent-sandbox-provision (no AWS/cloud "
    ...
    if not args.resume:
        description = (
            f"About to set up this machine for agent-sandbox-provision (user "
            f"{username}).\n"
    ...
    phases = [
        ...
        ("local_launchd",
         ...
         lambda: []),
        ("local_edit_mode_hooks",
         lambda: phase_local_edit_mode_hooks(ctx=ctx),
         lambda: phase_local_edit_mode_hooks_actions(ctx=ctx)),
    ]
    if ctx.dry_run:
        start = 0
    else:
    ...
    if remaining_actions:
        with common.admin_session(
            title="agent-sandbox-provision: machine setup (init)",
