...
def _git_credential_helper(user: str) -> str:
    ...
    return f"""#!/bin/sh
# claude-ro git credential helper (agent-sandbox-provision). Do NOT edit by hand.
set -eu
[ "$1" = "get" ] || exit 0
ORG=""
...
def _gh_shim(real_gh: str, user: str) -> str:
    ...
    return f"""#!/bin/sh
# claude-ro gh shim (agent-sandbox-provision). Do NOT edit by hand.
set -eu
REAL_GH="{real_gh}"
...
def main() -> None:
    ...
    with common.admin_session(
        title=f"agent-sandbox-provision: bind GitHub ({', '.join(new_orgs)})",
