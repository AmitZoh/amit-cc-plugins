"""
Shared helpers for agent-sandbox-provision sub-commands.
...
STATE_DIR = pathlib.Path(os.path.expanduser("~/.claude/plugins/data/agent-sandbox-provision"))
...
def prompt_yes_no(question: str, *, default: bool = False, ctx: Ctx | None = None) -> bool:
    ...
    if not sys.stdin.isatty():
        dialog_title = "agent-sandbox-provision"
        def _esc(s: str) -> str:
...
def prompt_secret(message: str, *, title: str = "agent-sandbox-provision") -> str:
...
def prompt_text(message: str, *, title: str = "agent-sandbox-provision") -> str:
...
def pick_from_list(prompt: str, items: list[str], *,
    ...
    if not sys.stdin.isatty():
        ...
        osa_line = (
            f'choose from list {{{listing}}} with title "agent-sandbox-provision" '
...
# claude-ro capabilities (agent-sandbox-provision). Printed at SessionStart. Lists
...
_AM_MINT_RULE = (
    ...
    "brokers installed by the agent-sandbox-provision skill, each reached through a sudoers "
