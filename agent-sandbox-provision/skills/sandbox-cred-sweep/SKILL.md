---
name: sandbox-cred-sweep
description: Sweep AWS surfaces (S3, Lambda, CFN, EC2 user-data, ECS, CodeBuild, Glue, SSM Parameter Store) for plaintext credentials reachable from the read-only sandbox identity. Reports locations only — never values. Runs INSIDE the agent-sandbox-provision sandbox (claude-ro); scans whatever account claude-ro was launched against.
user-invocable: true
...
The agent calls these directly via Python (`python3 -c "..."` or interactive Python). None require argparse / a CLI entry — they're library functions in `${CLAUDE_PLUGIN_ROOT}/skills/sandbox-cred-sweep/providers/aws.py`.
...
**From Skill 1 (the sibling `${CLAUDE_PLUGIN_ROOT}/skills/agent-sandbox-provision/` skill in this same plugin):**
...
```
# sandbox-cred-sweep — <ISO timestamp>
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/sandbox-cred-sweep/scripts/scan.py [--dry-run]
```
...
- **Playbook (`/sandbox-cred-sweep`)**: when the question is "is there an escalation path?" Slower, costs tokens, non-deterministic, can find chains regex never can.
...
- **Skill 1 (`agent-sandbox-provision`)** — the sibling skill under this same plugin — must be provisioned (i.e., `init` and `provision-account` for at least the account claude-ro is launched against; both skills install together via `/plugin install agent-sandbox-provision@amit-cc-plugins`, so nothing further to install separately). The playbook cross-imports Skill 1's `_common.py` and `providers/aws.py` (`surface_list`, `_client_error_code`, `_is_one_of`).
