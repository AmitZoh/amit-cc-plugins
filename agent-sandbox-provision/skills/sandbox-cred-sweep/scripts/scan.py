...
Runs INSIDE the agent-sandbox-provision sandbox (claude-ro), and only there.
...
helpers) from the sibling agent-sandbox-provision skill under this same
...
# path: parent -> scripts, parent.parent -> sandbox-cred-sweep, parent.parent.parent
# -> skills/, then across to agent-sandbox-provision.
SKILL1 = pathlib.Path(__file__).resolve().parent.parent.parent / "agent-sandbox-provision"
...
def _discover_identity(session: t.Any) -> tuple[str, str]:
    ...
    if len(parts) != 2:
        _bail(
            ...
            f"scan.py is only intended for the agent-sandbox-provision sandbox "
    ...
    if not role_name.startswith("claude-ro-"):
        _bail(
            ...
            f"agent-sandbox-provision sandbox identity — running it under your "
...
def main() -> None:
    ...
    print(f"# sandbox-cred-sweep run at {common.now_iso()}")
