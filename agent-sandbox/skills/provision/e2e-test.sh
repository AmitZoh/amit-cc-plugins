#!/usr/bin/env bash
# e2e-test.sh — interactive end-to-end test for agent-sandbox.
#
...
#
# Do NOT run other agent-sandbox commands in another terminal
...
sharing the account; delete them only when no engineer in the org is still using the sandbox.
  ...
  Delete via aws CLI / kubectl, or wait for revoke purge-org-fixtures (Skill 3, deferred).
