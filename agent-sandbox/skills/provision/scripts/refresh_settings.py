...
def _rerender_machine(username: str, ctx: common.Ctx) -> None:
    ...
    # First: repoint the stable skill link. Everything rendered below reaches the
    # skill through it, so after a rename, a version bump, or a move between a
...
def main() -> None:
    ...
    with common.admin_session(
        title="agent-sandbox: re-render local artifacts",
