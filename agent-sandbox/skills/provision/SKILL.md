...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/provision/scripts/init.py
...
python3 ${CLAUDE_PLUGIN_ROOT}/skills/provision/scripts/lockdown.py scan

# Phase 2: pipe verdicts to apply.
echo '{"verdicts": {"/path/to/file.key": "sensitive", "/other/keymap.json": "not_sensitive"}}' \
  | python3 ${CLAUDE_PLUGIN_ROOT}/skills/provision/scripts/lockdown.py apply
```
...
2. **Run `revoke delete` (or `delete-account`).** Removes the RO role; new `assume-role` calls fail. Existing session keys remain valid until they expire (capped at 12h, often less per Source-cred lifetime).
...
- Does NOT track binary symlinks placed at `/usr/local/bin/{claude,aws,kubectl}`. They're left in place on `revoke delete`.
