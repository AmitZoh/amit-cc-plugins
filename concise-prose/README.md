...
- A **Stop** hook (`hooks/stop-summary.py`) delivers a summary *out-of-band* at the end of substantive turns. It never blocks the main model. Flow:
  ...
  3. The filtered text is piped to `claude -p --bare --model haiku` (`--bare` skips hooks/plugins; `CONCISE_SUMMARIZER=1` is a second recursion guard). The summarizer may use only facts present in its input, and lists OPEN ITEMS only for questions the text explicitly poses.
